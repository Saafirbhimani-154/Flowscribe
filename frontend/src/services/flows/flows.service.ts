...
export const flowsService = {
   ...
   */
  uploadImages: async (files: File[]): Promise<UploadedFile[]> => {
  ...
  /**
   * Step 2: Analyze the flow. Images sent via FormData.
   * The upload step above gives immediate feedback to the UI
   * while the heavy LLM call happens.
   */
  analyzeFlow: async (files: File[], context: string = ''): Promise<FlowAnalyzeResponse> => {
    const formData = new FormData();
    files.forEach(file => formData.append('images', file));
